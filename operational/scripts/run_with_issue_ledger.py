...
def _packet(
    args: argparse.Namespace, outcome: str, summary: str, error_code: str | None = None
) -> dict[str, Any]:
    observed_at = _now()
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run a live child process and report process failures to the Issue Ledger."
    )
    parser.add_argument(
        "--db",
        type=Path,
    ...
    run.add_argument(
        "--record-success",
        action="store_true",
        help="also record successful process completion as report_only",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        try:
            ...
            _report(
                args, "failure", f"child command was not found: {child[0]}", "command_not_found"
            )
            return 127
        if return_code != 0:
            receipt = _report(
                ...
                f"process_exit:{return_code}",
            )
            print(
                f"issue-ledger-wrapper: failure reported stored={receipt['stored']}",
                file=sys.stderr,
            )
        elif args.record_success:
            receipt = _report(args, "success", "child process completed successfully")
            print(
                f"issue-ledger-wrapper: success observed stored={receipt['stored']}",
                file=sys.stderr,
            )
        return return_code

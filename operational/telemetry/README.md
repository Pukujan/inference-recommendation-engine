...
| repo-root `run_codex_harness.ps1 -InferHub` (dotsources `pc/astra-telemetry.ps1` + `pc/astra_otel.py`) | Same Astra OTel wrapper for cheap-agent / staff Codex InferHub-key runs (`llm.provider=inferhub`); non-InferHub runs stay untagged | developer PC |

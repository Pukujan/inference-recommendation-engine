...
def main() -> int:
    ...
    if not db or not agent_script:
        return 0
    session_id = (
        event.get("session_id") if isinstance(event.get("session_id"), str) else "unknown-session"
    )
    ...
    args = [
        ...
        "report",
        "--actor-id",
        f"agent:claude-code:{session_id}",
        "--actor-kind",
        "agent",
        "--harness",
        "claude-code",
        "--provider",
        os.environ.get("IRE_ISSUE_LEDGER_PROVIDER", "anthropic"),
        "--route",
        os.environ.get("IRE_ISSUE_LEDGER_ROUTE", "claude-code"),
        "--operation",
        "agent-session",
        "--workload-class",
        os.environ.get("IRE_ISSUE_LEDGER_WORKLOAD_CLASS", "long_horizon"),
        "--stream-mode",
        os.environ.get("IRE_ISSUE_LEDGER_STREAM_MODE", "unknown"),
        "--execution-id",
        f"claude-session:{session_id}",
        "--summary",
        "Claude Code reported an API failure",
        "--classification",
        "provider",
        "--outcome",
        "failure",
